 ...
 */
const withPersistentNotifications: ConfigPlugin<{
  /**
   * iOS location permission description
   */
  locationAlwaysAndWhenInUsePermission?: string;
  /**
   * iOS location when in use permission description
   */
  ...
  config = withAndroidManifest(config, (config) => {
    ...
    requiredPermissions.forEach((permission) => {
      if (
        !permissions.find((p) => p.$['android:name'] === permission)
      ) {
        permissions.push({
...
export default withPersistentNotifications;
